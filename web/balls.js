document.addEventListener('DOMContentLoaded', function() {
	...
	function setupPWAMode() {
		if (isPWA()) {
			...
			const darkBgPWA = document.getElementById('darkBg-pwa');
			const fps30PWA = document.getElementById('fps30-pwa');
			const originalModePWA = document.getElementById('originalMode-pwa');

			const darkBg = document.getElementById('darkBg');
			const fps30 = document.getElementById('fps30');
			const originalModeCheck = document.getElementById('originalMode');

			darkBgPWA.checked = darkBg.checked;
			fps30PWA.checked = fps30.checked;
			originalModePWA.checked = originalModeCheck.checked;

			darkBgPWA.addEventListener('change', function(e) {
				darkBg.checked = e.target.checked;
				darkBg.dispatchEvent(new Event('change'));
			});

			fps30PWA.addEventListener('change', function(e) {
				fps30.checked = e.target.checked;
				fps30.dispatchEvent(new Event('change'));
			});

			originalModePWA.addEventListener('change', function(e) {
				originalModeCheck.checked = e.target.checked;
				originalModeCheck.dispatchEvent(new Event('change'));
			});
		}

		window.matchMedia('(display-mode: window-controls-overlay)').addEventListener('change', setupPWAMode);
		window.matchMedia('(display-mode: standalone)').addEventListener('change', setupPWAMode);
	}
	...
	document.addEventListener('keydown', function(e) {
		if (e.key === 'h' || e.key === 'H') {
			...
			if (isPWA()) {
				if (titlebarControls.style.display === 'none') {
