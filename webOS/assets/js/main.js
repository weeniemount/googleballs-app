$(function() {
	...
	// Virtual cursor for webOS
	var virtualCursor = {
		x: 0,
		y: 0,
		speed: 5, // pixels per button press
		visible: true
	};
	...
	function init() {
		...
		virtualCursor.x = canvasWidth / 2;
		virtualCursor.y = canvasHeight / 2;
	...
	function initEventListeners() {
		...
		// webOS remote key event listeners
		document.addEventListener('keydown', handleWebOSKeys);
		
		// Keep mouse/touch support for testing
	...
	function handleWebOSKeys(e) {
		var keyCode = e.keyCode;
		var moved = false;
		
		// webOS Magic Remote keycodes
		switch(keyCode) {
			case 37: // Left arrow
				virtualCursor.x -= virtualCursor.speed;
				moved = true;
				break;
			case 38: // Up arrow
				virtualCursor.y -= virtualCursor.speed;
				moved = true;
				break;
			case 39: // Right arrow
				virtualCursor.x += virtualCursor.speed;
		...
		if (moved) {
			...
			virtualCursor.x = Math.max(0, Math.min(canvasWidth, virtualCursor.x));
			virtualCursor.y = Math.max(0, Math.min(canvasHeight, virtualCursor.y));
			
			// Update point collection with virtual cursor position
			if (pointCollection) {
				pointCollection.mousePos.set(virtualCursor.x, virtualCursor.y);
			}
	...
	function updateCanvasDimensions() {
		...
		// Recenter virtual cursor on resize
		virtualCursor.x = canvasWidth / 2;
		virtualCursor.y = canvasHeight / 2;
	...
	function drawVirtualCursor() {
		...
		// Horizontal line
		ctx.beginPath();
		ctx.moveTo(virtualCursor.x - 15, virtualCursor.y);
		ctx.lineTo(virtualCursor.x + 15, virtualCursor.y);
		ctx.stroke();
		
		// Vertical line
		ctx.beginPath();
		ctx.moveTo(virtualCursor.x, virtualCursor.y - 15);
		ctx.lineTo(virtualCursor.x, virtualCursor.y + 15);
		ctx.stroke();
		
		// Center dot
		ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
		ctx.beginPath();
		ctx.arc(virtualCursor.x, virtualCursor.y, 3, 0, Math.PI*2, true);
		ctx.fill();
