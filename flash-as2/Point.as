class Point {
	...
	public function draw(mc:MovieClip) {
		...
		var y = this.curPos.y;
    	with (mc) {
	        beginFill(this.color.toNumber(), 100);
    	    moveTo(x + this.radius, y);
	        curveTo(this.radius + x, Math.tan(Math.PI / 8) * this.radius + y, Math.sin(Math.PI / 4) * this.radius + x, Math.sin(Math.PI / 4) * this.radius + y);
    	    curveTo(Math.tan(Math.PI / 8) * this.radius + x, this.radius + y, x, this.radius + y);
        	curveTo(-Math.tan(Math.PI / 8) * this.radius + x, this.radius+ y, -Math.sin(Math.PI / 4) * this.radius + x, Math.sin(Math.PI / 4) * this.radius + y);
        	curveTo(-this.radius + x, Math.tan(Math.PI / 8) * this.radius + y, -this.radius + x, y);
        	curveTo(-this.radius + x, -Math.tan(Math.PI / 8) * this.radius + y, -Math.sin(Math.PI / 4) * this.radius + x, -Math.sin(Math.PI / 4) * this.radius + y);
        	curveTo(-Math.tan(Math.PI / 8) * this.radius + x, -this.radius + y, x, -this.radius + y);
        	curveTo(Math.tan(Math.PI / 8) * this.radius + x, -this.radius + y, Math.sin(Math.PI / 4) * this.radius + x, -Math.sin(Math.PI / 4) * this.radius + y);
        	curveTo(this.radius + x, -Math.tan(Math.PI / 8) * this.radius + y, this.radius + x, y);
