...
# How 2 compile gtk
Go inside of ``native-gtk`` and if ur on windows you have to install msys and install gtk. for linux just install gtk. on either os run `make` and it should hopefully compile (makefile only tested on fedora 42 xfce running sway (i use fedora btw - ploszukiwacz)).
